...
- 在网页图片左上角显示快捷识别按钮
- 点击扩展图标即可打开面板，支持直接输入提示词生图
- 支持上传、粘贴或拖入本地图片后识别反推提示词
- 点击图片按钮后自动分析图片内容
- 生成精简版 / 完整版提示词
- 支持中文 / 英文提示词切换
- 支持结构化查看提示词内容
...
> 说明：浏览器扩展无法做到绝对隐藏源码，只能通过混淆提高阅读成本。
...
- `dist/`：混淆后的扩展目录（可直接「加载已解压的扩展程序」）
- `release/xiaoboDraw-v版本号.zip`：可发给别人的安装包

对方安装：解压 zip → Chrome 打开 `chrome://extensions/` → 开启开发者模式 →「加载已解压的扩展程序」→ 选择解压后的文件夹。
